...
from Yone.Database.users_sql import get_user_com_chats, get_all_chats
from Yone.Handlers.decorators import Yonemsg
from Yone import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of the Yone Team!\nI can't act against our own."
        )
        return
    ...
    if int(user_id) in REQUESTER:
        message.reply_text(
            "Trying to gban a Yone servant?, you got a death wish or something?")
        return
...
*Note:* Users can appeal gbans or report spammers at `@Yone_Support`."""
